...
func TestGetStatePasswordMissingFile(t *testing.T) {
	...
	_, err := command.getStatePassword("0")
	if err == nil {
		t.Errorf("Expected 'file not found' error")
	}
	if err.Error() != "opening agent config file \"/var/lib/juju/agents/machine-0/agent.conf\": file not found" {
...
juju-db-snap-channel: 4.4/stable`
	...
	command := &rebootstrapCommand{}
	password, err := command.getStatePassword("0")
	if err != nil {
